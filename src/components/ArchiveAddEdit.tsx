...
export default function ArchiveAddEdit({
  ...
  // --- RBAC: kategori yang boleh dipilih user (mis. Notaris -> AJB & APP saja) ---
  ...
  // --- RBAC: kategori default = kategori pertama yang diizinkan user ---
  const [kategori, setKategori] = useState<KategoriArsip>(
    allowedKategori[0] ?? KategoriArsip.AKTA_JUAL_BELI
  );
  ...
  useEffect(() => {
    if (mode === "edit" && existingArchive) {
      ...
      // --- RBAC: kategori awal = kategori pertama yang diizinkan ---
      const initialKategori = allowedKategori[0] ?? KategoriArsip.AKTA_JUAL_BELI;
      setKategori(initialKategori);
      setStatusArsip(StatusArsip.AKTIF);
      setMasaRetensi(systemConfig.defaultRetensi[initialKategori] as number);
      setNamaKlien("");
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  disabled={allowedKategori.length <= 1}
                  className="w-full bg-white border border-[#D4B896] focus:border-gold-royal focus:outline-none rounded-lg p-3 text-xs text-[#0B1F3A] cursor-pointer disabled:bg-[#FAFAF8] disabled:cursor-not-allowed"
                >
                  {allowedKategori.map((cat) => (
                    <option key={cat} value={cat}>{cat}</option>
                  ))}
                </select>
                {allowedKategori.length < Object.values(KategoriArsip).length && (
                  <span className="text-[9px] text-[#718096] leading-none block">
                    Peran Anda hanya dapat menambah arsip pada kategori di atas.
                  </span>
                )}
              </div>
